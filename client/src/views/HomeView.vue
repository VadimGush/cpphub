...
<template>
  <div class="mx-5 mb-5 d-flex flex-row authors">
    <AuthorSpot v-for="author in authors" :picture="author.icon" @click="(e) => openAuthor(author, e)">{{ author.name }}
    </AuthorSpot>
  </div>

  <div class="mx-5 categories">
    <Button v-for="category in categories" :large="true" :key="category.id" :icon="category.icon"
      :counter="category.size" @click="(e) => openCategory(category, e)">{{ category.name }}</Button>
  </div>
</template>
